...
void write_header_binary(const irap_header& header, std::ostream& out) {
  ...
  write_32bit_binary_value(-996, out);
  ...
  for (size_t j = 0; j < cols; j++) {
    for (size_t i = 0; i < rows; i++) {
      if (written_on_line == 0) {
        guard_length = std::min(len - (j * cols + i), PER_LINE_BINARY);
        write_32bit_binary_value(guard_length * 4, out);
      }
      ...
      if (++written_on_line == guard_length) {
        write_32bit_binary_value(guard_length * 4, out);
        written_on_line = 0;
      }
    }
